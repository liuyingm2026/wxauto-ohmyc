...
def _scroll_for_unread(wx, max_pages=10):
    """滚动会话列表查找未读会话（WeChat 3.9.11 SessionBox 虚拟化兜底）。

    微信 UIA SessionBox 只暴露前 ~10 个可见项。当目标会话不在可见区域时，
    GetSessionList(newmessage=True) 返回空。此函数逐页滚动查找所有未读会话。
    """
    all_unread = {}
    seen_names = set()

    # 先点击 SessionBox 确保键盘焦点在列表上
    try:
        wx.SessionBox.Click(simulateMove=False)
    except Exception:
        pass
    time.sleep(0.15)
    ...
    for page in range(max_pages):
        # 获取当前页面的所有会话
        try:
            current = wx.GetSessionList(newmessage=True)
        except Exception:
            current = {}

        if current:
            all_unread.update(current)
            logger.debug("_scroll_for_unread: 第 %d 页发现 %d 个未读会话",
                         page + 1, len(current))

        # 获取当前所有可见会话名，判断是否已到列表尾部
        try:
            all_current = wx.GetSessionList()
            new_names = set(all_current.keys()) - seen_names
            if not new_names and page > 0:
                break
            seen_names.update(all_current.keys())
        except Exception:
            pass
        ...
        try:
            ...
            # pyautogui 不可用时用 UIA PageDown 兜底
            wx.UiaAPI.SendKeys('{PageDown}', waitTime=0.3)
            time.sleep(0.4)

    if all_unread:
        logger.info("_scroll_for_unread: 共发现 %d 个未读会话（扫描 %d 页）",
                    len(all_unread), page + 1)
    return all_unread
...
def _read_sessionbox_preview(wx, max_pages=5):
    ...
    SessionBox ListItemControl 结构 (WeChat 3.9.11):
      ListItemControl Name='测试自动回复3条新消息'
        PaneControl
          ButtonControl Name='测试自动回复'        → session name
          TextControl '测试自动回复'               → duplicate name
          TextControl '9:47'                      → timestamp
          TextControl 'SenderName：Content'       → message preview
          TextControl '3'                         → unread badge

    私聊格式: "SenderName：Content"
    群聊格式: "SenderName：Content" (相同)
...
def process_unread(wx, seen, rl: RateLimiter, ss: SessionStore):
    ...
    try:
        ...
        # 如果仍然为空，滚动会话列表查找未读会话
        # WeChat 3.9.11 SessionBox 虚拟化，只有前 10 个可见项
        if not sessions:
            sessions = _scroll_for_unread(wx)
        # 节流日志：只在会话列表变化时输出
        ...
        err_msg = str(e).lower()
        # UIA COM 错误（RDP 断开、窗口重启等导致连接过期），通知主循环重连
        if '事件无法调用任何订户' in err_msg or 'com' in str(type(e).__name__).lower():
            return "reconnect"
        return seen
    ...
    # 预读会话列表消息预览，用于 GetAllMessage 返回空时的兜底
    # RDP 断开时 ChatBox.C_MsgList 不可见，GetAllMessage() 返回空，
    # 但 SessionBox 子控件始终可用，可从中提取最后一条消息预览
    _sessionbox_previews = _read_sessionbox_preview(wx)
    ...
    for who in pending_sessions:
        ...
        if not msgs:
            # ── SessionBox 预览兜底 ──────────────────────
            # RDP 断开时 ChatBox.C_MsgList 不可见，GetAllMessage 返回空。
            # 从 SessionBox UIA 子树读取最后一条消息预览作为替代。
            preview = _sessionbox_previews.get(who)
            if preview:
                sender, content_preview = preview
                if not content_preview or not content_preview.strip():
                    logger.debug("Phase1: 预览内容为空 %s", who[:20])
                    _return_to_session_list()
                    continue
                if sender == 'Self':
                    logger.debug("Phase1: 预览跳过 Self %s", who[:20])
                    _return_to_session_list()
                    continue

                fp = msg_fingerprint(who, sender, content_preview.strip())
                if fp in updated_seen:
                    logger.debug("Phase1: 预览重复 %s fp=%s", who[:20], fp[:8])
                    _return_to_session_list()
                    continue
                ...
                logger.info("Phase1 预览采集 [%s] %s: %s",
                            who[:20], sender[:20], content_preview[:60])
            else:
                logger.debug("Phase1: 无预览 %s", who[:20])
            _return_to_session_list()
            continue
    ...
    while running:
        try:
            ...
            if rl.is_active_hours():
                ...
                if new_seen == "reconnect":
                    logger.warning("UIA COM 连接过期，尝试重连微信...")
                    try:
                        from wxauto import WeChat
                        wx = WeChat(language="cn")
                        logger.info("重连成功: %s", getattr(wx, "nickname", "ok"))
                        _last_force_refresh = 0  # 强制刷新会话列表
                        error_count = 0
                    except Exception as recon_err:
                        logger.error("重连失败: %s", recon_err)
                        error_count += 1
                        time.sleep(30)
                    continue
